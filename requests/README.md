## request

- [What are the differences between the urllib, urllib2, and requests module?](https://stackoverflow.com/questions/2018026/what-are-the-differences-between-the-urllib-urllib2-and-requests-module)
- [python中urllib, urllib2,urllib3, httplib,httplib2, request的区别](https://blog.csdn.net/permike/article/details/52437492)
- [urllib3](https://github.com/urllib3/urllib3)
- [requests](https://github.com/requests/requests)
- [hyper](https://github.com/Lukasa/hyper)
- [grequests](https://github.com/kennethreitz/grequests)
- [patch_all](http://www.gevent.org/_modules/gevent/monkey.html#patch_all)
- [requests-threads](https://github.com/requests/requests-threads)
- [requests-twisted](https://github.com/sametmax/requests-twisted)
- [requests-futures](https://github.com/ross/requests-futures)

## http presistent connection

- [http的长连接和短连接（史上最通俗！）](https://www.jianshu.com/p/3fc3646fad80)